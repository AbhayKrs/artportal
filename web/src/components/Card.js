...
export const ImageCard = ({ size, artwork, artist }) => {
    let navigate = useNavigate();

    return (
        <div className={`group flex relative cursor-pointer ${size === 'l' ? 'h-44 w-auto' : 'h-36 w-auto'}`} onClick={() => navigate(`/library/${artwork._id}`)}>
            {artwork.files[0].length > 0 && <img src={api_artworkImages(artwork.files[0])} className='object-cover w-full h-full' />}
...
export const HorizontalCard = ({ artwork, artist }) => {
    let navigate = useNavigate();
    ...
    return (
        <div className='flex h-36 relative'>
            <div className='flex absolute left-0 h-full w-[50%] rounded-l-lg' style={{
                ...
                backgroundImage: `url(${api_artworkImages(artwork.files[0])})`,
            ...
            <div className='flex flex-col absolute right-0 h-full w-[60%] rounded-r-lg text-gray-700 dark:text-gray-300 backdrop-sepia-0 bg-slate-300/75 dark:bg-neutral-700/25' style={{ clipPath: 'polygon(17% 0%, 100% 0%, 100% 100%, 0% 100%)' }}>
                <div className='flex flex-col max-w-[85%] h-full w-full p-4 absolute inset-y-0 right-0 m-auto justify-between'>
                    <div className='gap-2'>
                        <h3 className="lg:text-lg md:text-md sm:text-sm font-bold lg:leading-5 md:leading-5 sm:leading-5 capitalize">{artwork.title.length > 20 ? artwork.title.slice(0, 20) + "..." : artwork.title}</h3>
                    ...
                    <div className="flex flex-row justify-between">
                        ...
                        <BsInfoCircleFill onClick={() => navigate(`/library/${artwork._id}`)} className="cursor-pointer ml-auto h-5 w-5" />
                    </div>
                </div>
            </div>
        </div>
        // <div style={{ backgroundImage: `url(${api_artworkImages(artwork.files[0])})` }} className="bg-gray-100 m-auto w-full h-64 bg-top bg-no-repeat bg-cover">
        //     <div className="hidden items-end h-full w-full group-hover:flex group-hover:flex-row">
        //         <div className="flex flex-col w-full pb-2 pt-14 px-2 bg-gradient-to-t from-black text-gray-200 group-hover:flex" >
        //             <h3 className="text-base font-bold leading-5 capitalize">{artwork.title}</h3>
        //             <div className="inline-flex items-center">
        //                 <div className="w-5 h-5 overflow-hidden mr-1">
        //                     <img loading='lazy' src={artist && api_userImages(artist.avatar.icon)} alt="user_avatar" className="object-cover w-full h-full" />
        //                 </div>
        //                 <span className="font-base text-xs my-1 mr-1">
        //                     {artist.username}
        //                 </span>
        ...
        //                     </div>
        //                 </div>
        //                 <button onClick={() => navigate(`/library/${artwork._id}`)} className="w-5 h-5 bg-gray-200 text-gray-900 rounded-full hover:bg-gray-300 active:shadow-lg shadow transition ease-in duration-200 focus:outline-none">
