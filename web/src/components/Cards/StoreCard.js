...
const StoreCard = ({ size, artwork, artist }) => {
    let navigate = useNavigate();

    return (
        <div className={`flex-none rounded-md cursor-pointer ${size === 'l' ? 'h-52 w-full' : 'h-40 w-full'}`} onClick={() => { }}>
            {artwork.files[0].length > 0 && <img src={api_artworkImages(artwork.files[0])} className='object-cover h-36 w-full rounded-md' />}
