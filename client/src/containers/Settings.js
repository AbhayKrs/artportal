        ...
        <div className='main-container flex flex-col gap-2 p-4 bg-gray-200 dark:bg-darkNavBg'>
            ...
            <div className='grid grid-cols-4 gap-4'>
                <div className='flex flex-col p-3 space-y-3 bg-slate-300 rounded-lg dark:bg-neutral-900 '>
                    ...
                    <button disabled className={childPath === 'notifications' ? `text-purple-500` : `text-gray-500 dark:text-gray-600`} onClick={() => navigate('/settings/notifications')}>Notifications</button>
                    <button disabled className={childPath === 'community' ? `text-purple-500` : `text-gray-500 dark:text-gray-600`} onClick={() => navigate('/settings/community')}>Community</button>
                    <button disabled className={childPath === 'billing' ? `text-purple-500` : `text-gray-500 dark:text-gray-600`} onClick={() => navigate('/settings/billing')}>Billing</button>
                </div>
                <Outlet />
            </div>
        </div>
